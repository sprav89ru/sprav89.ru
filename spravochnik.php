...
echo '<link rel="stylesheet" href="'.$path.'/сss/styles.css" type="text/css"/>'; 
	...
	//echo ' $_POST[link] = '.$_POST['link'];
	
	if (isset($_POST['source'])) {
		//echo ' $_POST[link] = '.$_POST['source'];
		//
		//$_POST['source']=PATH.'/?option=sprav&';
		}
	else {
		$_POST['source']=''.PATH.'/spravochnik.php?';
		//$navy_link0=
		 }

/*	if (isset($_GET['link'])) {
		$link=''; //откуда идет вызов
		}
	else {
		$link=''; 
		 }
*/	
	if (isset($_GET['idr'])) { $idr=$_GET['idr']; }
	else { $idr=1; }
	
	if (isset($_GET['zn'])) { $zn=$_GET['zn']; }
	else { $zn=1; }
	...
	case 4:
		...
		break;
	default:
		//echo "функция уровня 1 ";
		f_ur1($mysqli); // выводит 1-й уровень
	break;
	}	

function f_ur1($mysqli, $idr)  // выводит группы 1-го уровня
	{	
	...
	while ($row = mysqli_fetch_assoc($res1))
		...
		$link=''.$_POST['source'].'zn='.$zn.'&idr='.$id;
		echo '<div class="group1"><a class="a_group1" href=' . $link .'>'.  $kod . '. ' . $naimenovanie_na_pechat .'</a></div>';
		
			$q2 ='SELECT * FROM roditel2 WHERE idr ="'.$id.'" ORDER BY kod';
		...
		while ($row = mysqli_fetch_assoc($res2)) 
			...
			//$link='spravochnik.php?zn='.$zn.'&idr='.$id;
			$link=''.$_POST['source'].'zn='.$zn.'&idr='.$id;
			echo '<div class="group2"><a class="a_group2" href=' . $link .'>'.  $kod . '. ' . $naimenovanie_na_pechat .'</a></div>';
			}
		}
		//mysqli_close($mysqli);
	}

function f_ur2($mysqli, $idr)  // выводит группы 2-го уровня + элементы
	{	
	
			$q ='SELECT * FROM roditel1 WHERE id ="'.$idr.'" ORDER BY kod LIMIT 1';
		//echo $q;
		$res = mysqli_query($mysqli, $q) or die(mysqli_error());
		$row = mysqli_fetch_array($res); //выберем одну строку (она и должна быть одна)
		
		//echo ' $row[kod]  = '.$row[0].' $row[idr]  = '.$row[1].' $row[id]  = '.$row[2].' $row[naimenovanie_na_pechat]  = '.$row['naimenovanie_na_pechat'].' $row[name]  = '.$row['name'];
		//echo ' $row[kod]  = '.$row['kod'].' <br/ > $row[idr]  = '.$row['idr'].' <br/ > $row[id]  = '.$row['id'].' <br/ >  $row[naimenovanie_na_pechat]  = '.$row['naimenovanie_na_pechat'].' <br/ >  $row[name]  = '.$row['name'];
		
		$link=''.$_POST['source'].'zn=2&idr='.$row['id'];
		
		echo '<div class="group1"><a class="a_group1" href=' . $link .'>'.  $row['kod'] . '. ' . $row['naimenovanie_na_pechat'] .'</a></div>';

	$q2 ='SELECT * FROM roditel2 WHERE idr ="'.$idr.'" ORDER BY kod';
	$res2 = mysqli_query($mysqli, $q2) or die(mysql_error());
	while ($row = mysqli_fetch_assoc($res2)) 
		{
		$kod=$row['kod'];
...
function f_ur3($mysqli,$idr)//Элементы группы $idr
	{
	//	$_POST['zn'] = 3;
	//	$_POST['idr'] = $idr;
		
		//require_once 'navy.html';	
		$q ='SELECT * FROM roditel2 WHERE id ="'.$idr.'" ORDER BY kod LIMIT 1';
		//echo $q;
		$res = mysqli_query($mysqli, $q) or die(mysqli_error());
		$row = mysqli_fetch_array($res); //выберем одну строку (она и должна быть одна)
		...
		$link=''.$_POST['source'].'zn=3&idr='.$idr;
		
		echo '<div class="group1"><a class="a_group1" href=' . $link .'>'.  $row['kod'] . '. ' . $row['naimenovanie_na_pechat'] .'</a></div>';
		
		$q1 ='SELECT * FROM sprav WHERE idr ="'.$idr.'" ORDER BY priority, kod';
		$res1 = mysqli_query($mysqli, $q1) or die(mysqli_error($mysqli));
		while ($row = mysqli_fetch_assoc($res1))
			{	
		//echo ' $mysqli = '.mysqli_info($mysqli).' $row[id] '.$row['id'].' <br>';
			f_ur4($mysqli, $row['id']);
			}
		//mysqli_close($mysqli);
	}
	
function f_ur4($mysqli,$id) // Данные элемента с id
{
	$q ='SELECT * FROM sprav WHERE id ="'.$id.'" ORDER BY kod';
	$res = mysqli_query($mysqli, $q) or die( mysqli_error($mysqli) );
	$row = mysqli_fetch_array($res); //выберем одну строку (она и должна быть одна)
	
	$link=''.$_POST['source'].'zn=4&idr='.$id;
	...
	$q_tel ='SELECT * FROM sprav_tel WHERE idr ="'.$id.'" ORDER BY nom_str';
	...
	{ //цикл по телефонам
		...
		$q ='SELECT * FROM sprav_dop WHERE idr ="'.$id.'" ORDER BY id';
	...
	//mysqli_close($mysqli);
}//function f_ur4($idr) // Данные элемента 

//функция форматирования телефонов
function  tel($tel){
